...
def build_and_solve_MMPDP_model(data=None):
    """
    建立模型，求解模型并可视化
    :param data:
    :return:
    ...
    for i in range(1, data.node_num):
        ...
        PDVRP.addConstr(expr1 == 1, name='cons1_' + str(i))

    # 约束 2 车辆流平衡约束，保证了每一辆车进出每个节点的次数相等
    for i in range(data.node_num):
        for k in range(vehicle_num):
            expr2 = LinExpr()
            for j in range(data.node_num):
            ...
            PDVRP.addConstr(expr2 == 0, name='cons2_' + str(i) + "_" + str(k))
    ...
    for i in range(data.node_num):
        for j in range(data.node_num):
            if i != j:
                ...
                PDVRP.addConstr(expr3 <= 0, name='cons3_' + str(i) + "_" + str(j))
    ...
    for i in range(1, data.node_num):
        ...
        PDVRP.addConstr(expr4 == data.demand[i], name='cons5_' + str(i))
    ...
    nx.draw_networkx(Graph, pos_location, node_size=200, node_color=nodes_color_dict, edge_color=edge_color_dict0,
                     ...
                     font_size=8)
    # 保存图片
    plt.savefig("fig_MMpdp.pdf", dpi=800)
    plt.legend()
    plt.show()
...
if __name__ == '__main__':
    ...
    # 调用函数建模并求解
    build_and_solve_MMPDP_model(data=data)
