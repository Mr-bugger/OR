        ...
        try:
            ...
            for k in range(data.vehicle_num):
                for i in range(data.vehicle_route_num):
                    for j in range(data.vehicle_route_num):
                        if i != j:
                            self.model.addConstr(self.t[N - 1, i] + data.vehicle_reload_time - self.t[0, j] <= (3 - self.z[i, j, k] - self.y[k, i] - self.y[k, j]) * self.big_M,
                                        "DisjunctiveLeft_%d_%d_%d" % (i, j, k))
                            self.model.addConstr(self.t[0, i] - data.vehicle_reload_time - self.t[N - 1, j] >= -(2 + self.z[i, j, k] - self.y[k, i] - self.y[k, j]) * self.big_M,
                                        "DisjunctiveLeft_%d_%d_%d" % (i, j, k))
                                        ...
                                        for j in range(1, N):
                                            if current_node != j and self.x[current_node, j, r].x >= 0.9:
                                                if j != N - 1:
                                                    ...
                                                    print("场站(%s)" % round(self.t[j, r].x), end="\n")
                                                    flag = False
                                                break
                                    Count += 1
