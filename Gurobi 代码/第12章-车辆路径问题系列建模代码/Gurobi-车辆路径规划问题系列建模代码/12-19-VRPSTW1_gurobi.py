...
class Data(object):
    ...
    def read_and_print_data(self, path=None, data=None):
        """
    ...
    def build_and_solve_model(self, data=None):
        ...
        try:
            ...
            for i in range(N):
                ...
                if i in range(1, N - 1):
                    self.t[i] = self.model.addVar(lb=data.early_time_window[i], ub=data.late_time_window[i] + 10,
            ...
            for i in range(N):
                print("Name:%d , value:%d" % (i, self.beta[i].x), end="\n")
