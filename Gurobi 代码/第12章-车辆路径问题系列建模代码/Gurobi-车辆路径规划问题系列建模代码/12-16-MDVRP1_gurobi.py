...
class Model_builder(object):
    ...
    def build_and_solve_model(self, data=None):
        ...
        # 开始在Gurobi中建模
        try:
            self.model = Model("MDVRP1")

            # 创建变量
            # 将第一个点和第二个点作为都作为车场。
            for i in range(data.node_num):
                # 创建车辆到达点i剩余容量变量q_i,是连续类型变量，算例设置的车辆容量最大为200，因只使用了其中14个点，将容量限制为80。
                self.q[i] = self.model.addVar(lb=0.0, ub=80, obj=0.0, vtype=GRB.CONTINUOUS, name="q_%d" % i)
                for j in range(data.node_num):
                    # 默认不存在点i到点i的弧。
                    if i != j:
                        # 以下两个是排除车场到车场的变量
                        if i + j < data.depot_num:
                            continue
                        # 以{d}结尾的弧
                        elif i == data.depot_num - 1 and j == data.depot_num - 1:
                            continue
                        # else的是所有可能的弧
                        else:
            ...
            for i in range(0, data.depot_num):
                expr = LinExpr()
                for j in range(data.depot_num, data.node_num):
                    if i != j:
            ...
            for i in range(data.depot_num, data.node_num):
                expr = LinExpr()
                for j in range(data.node_num):
                    if i != j:
                        expr.addTerms(1, self.x[i, j])
            ...
            for k in range(data.depot_num):
                self.model.addConstr(
                    sum(self.x[k, j] for j in range(2, data.node_num)) - sum(
                        self.x[i, k] for i in range(2, data.node_num)) == 0,
                    "DepotFlowConstr%d" % k)
            ...
            for j in range(data.depot_num):
                for i in range(data.depot_num, data.node_num):
                    self.model.addConstr(self.q[i] + data.demand[i] - 80 <= (1 - self.x[i, j]) * 200,
                                         "Capacity_%d_%d" % (i, j))
