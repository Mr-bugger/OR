...
class Data(object):
    ...
    def read_and_print_data(self, path, data):
        ...
        # 计算距离矩阵，保留两位小数，并打印矩阵
        data.dis_matrix = [[round(math.sqrt(sum((ori_data[i][k] - ori_data[j][k]) ** 2 for k in range(1, 3))), 2) for i in range(data.node_num)]
        ...
        try:
            ...
            for k in range(data.vehicle_num):
                for i in range(data.node_num + 1):
                    ...
                    self.q[i, k] = self.model.addVar(lb=0.0, ub=100, obj=0.0, vtype=GRB.CONTINUOUS, name="q_%d_%d" % (i, k))
                for i in range(data.node_num):
            ...
            for k in range(data.vehicle_num):
                ...
                self.model.addConstr(sum(self.x[0, j, k] for j in range(1, data.node_num)) <= 1, "DepotFlowConstr2_%d" % k)
            ...
            for k in range(data.vehicle_num):
                for i in range(1, data.node_num):
                    for j in range(1, data.node_num + 1):
                        if i != j:
                            self.model.addConstr(self.q[i, k] + data.demand[i] - self.q[j, k] <= (1 - self.x[i, j, k]) * 200,
