...
class Model_builder(object):
    ...
    def build_and_solve_model(self, data=None):
        ...
        try:
            ...
            for k in range(data.vehicle_num):
                ...
                for i in range(data.vehicle_route_num):
                    for j in range(data.vehicle_route_num):
                        if i != j:
                            self.z[i, j, k] = self.model.addVar(obj=0.0, vtype=GRB.BINARY, name="z_%d_%d_%d" % (i, j, k))
            ...
            for r in range(data.vehicle_route_num):
                self.model.addConstr(quicksum(self.x[0, j, r] for j in range(1, N - 1))
                                     - quicksum(self.x[i, N - 1, r] for i in range(1, N - 1)) == 0,
            ...
            for r in range(data.vehicle_route_num):
                ...
                for i in range(1, N - 1):
                    for j in range(1, N):
                        if i != j:
                            sum.addTerms(data.demand[i], self.x[i, j, r])
                self.model.addConstr(sum <= 75, "Capacity_%d" % r)
            ...
            for k in range(data.vehicle_num):
                ...
                for r in range(data.vehicle_route_num):
                    lh.addTerms(1.0, self.y[k, r])
                self.model.addConstr(lh <= self.f[k] * 500, "UseVehicle_%d" % k)
            ...
            for k in range(data.vehicle_num):
                for i in range(data.vehicle_route_num):
                    for j in range(data.vehicle_route_num):
                        if i != j:
                            self.model.addConstr(self.t[N - 1, i] + data.vehicle_reload_time - self.t[0, j] <= (1 - self.z[i, j, k]) * self.big_M,
            ...
            for k in range(data.vehicle_num):
                ...
                for i in range(data.vehicle_route_num):
                    for j in range(data.vehicle_route_num):
                        if i != j:
                            rh.addTerms(1.0, self.z[i, j, k])
                self.model.addConstr(lh - 1 <= rh, "ArcAssignment_%d" % k)
            ...
            for k in range(data.vehicle_num):
                for i in range(data.vehicle_route_num):
                    ...
                    for j in range(data.vehicle_route_num):
                        if i != j:
                            rh1.addTerms(1.0, self.z[j, i, k])
                            rh2.addTerms(1.0, self.z[i, j, k])
                    self.model.addConstr(self.y[k, i] >= rh1, "ArcFlowRight_%d_%d" % (i, k))
                    self.model.addConstr(self.y[k, i] >= rh2, "ArcFlowLeft_%d_%d" % (i, k))
